...
def _load_wfi_detector_aberrations(filename):
    ...
    # (change references to these variables back once done testing)
    if 'Z210' in zernike_table.colnames:  # newer files
        det_col = 'sca'
        fp_col = 'fov'
        wave_col = 'wave'
        det_pos_x = 'SCI_X'  # already in pixels
        det_pos_y = 'SCI_Y'  # already in pixels
        calc_pix = False
        conv_to_m = 1e-9  # nm to m. this is wavelength unit, not Zernike term
    else:
        det_col = 'sca'
        fp_col = 'field_point'
        wave_col = 'wavelength'
        det_pos_x = 'local_x'  # still in mm
        det_pos_y = 'local_y'  # still in mm
        calc_pix = True
        conv_to_m = 1e-6  # micron to m
    ...
    def build_detector_from_table(number, zernike_table):
        ...
        for field_id in field_points:
            ...
            local_x, local_y = field_point_rows[0][det_pos_x], field_point_rows[0][det_pos_y]
            interpolator = build_wavelength_dependence(field_point_rows,
                                                       n_zernikes)

            if not calc_pix:  # newer files
                # TEMP: remove "if" and unindent below (if kept. probably so)
                pixx, pixy = local_x, local_y  # already in pixels
            else:
                # TEMP: delete "else" and unindent below (if kept. probably not)
...
class WFI(RomanInstrument):
    ...
    def __init__(self, datapath=None):
        ...
        if datapath:
            ...
            # may need to shift where self.detector is set?
    ...
    def aperturename(self, value):
        ...
        for fl, info in self._filters.items():
            ...
            if self._datapath != os.path.join(self._STPSF_basepath, self.name):
                # TEMP: remove "if" when done testing
                info_edit = {key : (val if key != 'filename'
                                    else str(Path(self._datapath) / 'filters' / self._detector / filter_path.name))
                             for key, val in info._asdict().items()}
            else:
                # TEMP: unindent "else" when done testing
                info_edit = {key : (val if key != 'filename'
